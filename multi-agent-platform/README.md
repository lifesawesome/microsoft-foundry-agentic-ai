...
| --- | --- | --- | --- |
| Orchestration | Agent Framework workflow | `WorkflowBuilder` graph: router → knowledge / action / response | Parallel fan-out, checkpoints |
| Enterprise data | `KnowledgeProvider` | Foundry IQ / Azure AI Search | SharePoint, OneDrive, Blob, ADLS, Data Lake Data Agents |
| Tools & actions | `ToolProvider` | Read-only web search + simulated side-effecting `create_case`, policy-gated | Real Bing/MCP, social, external APIs |
| User channels | `ChannelAdapter` | REST API + web widget | Microsoft Teams |
| Conversation state | `SessionStore` | Redis (in-memory for dev) | — |
| Acting on behalf of a user | `DelegatedIdentityProvider` | Boundary wired, simulated token | Real OBO / token-exchange actions |
